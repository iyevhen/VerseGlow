...
{
	internal class nsEUCTWProber : AbstractCSProber
	{
		//void      GetDistribution(PRUint32 aCharLen, const char* aStr);

		private readonly CodingStateMachine mCodingSM;

		//private EUCTWContextAnalysis mContextAnalyser;
		private readonly EUCTWDistributionAnalysis mDistributionAnalyser = new EUCTWDistributionAnalysis();
			...
			for (int i = 0; i < aBuf.Length && i < aLen; i++)
			{
				...
				if (codingState == SMState.Start)
				{
					...
					if (i == 0)
					{
						...
						mDistributionAnalyser.HandleOneChar(new[] {aBuf[i - 1], aBuf[i]}, charLen);
				}
			}
