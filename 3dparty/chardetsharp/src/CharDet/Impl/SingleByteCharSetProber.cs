...
{
	internal class SingleByteCharSetProber : AbstractCSProber
	{
		...
		private readonly SequenceModel mModel;
		private readonly bool mReversed; // PR_TRUE if we need to reverse every pair in the model lookup

		//char order of last character
		private byte mLastOrder;

		private int mTotalSeqs;
		private readonly int[] mSeqCounters = new int[NUMBER_OF_SEQ_CAT];

		private int mTotalChar;
		//characters that fall in our sampling range
		private int mFreqChar;

		// Optional auxiliary prober for name decision. created and destroyed by the GroupProber
		private readonly AbstractCSProber mNameProber;
		...
		public bool KeepEnglishLetters()
		{
