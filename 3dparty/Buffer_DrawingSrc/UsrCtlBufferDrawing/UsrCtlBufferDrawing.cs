...
namespace BufferDrawing
{
	public partial class UsrCtlBufferDrawing : UserControl
	{
		...
		private readonly SolidBrush brushcircle;
		private readonly SolidBrush brushdotnormal;
		private readonly SolidBrush brushdotselected;
		private readonly BufferedGraphics[] buffergraphs;
		private readonly Pen pencircle;
		private readonly Pen pendotborder;
		...
		private Boolean[] selecpto;
		public TimeSpan timeused;
		public bool uselayers;

		private TypeOfChange vartchange;
		...
		{
			string text;
			var textFont = new Font("Arial", 9 + dotwidth*2);
			...
			while (fromY <= r.Height)
			{
				...
				while (fromX <= r.Width)
				{
					...
					var textLocation = new PointF(fromX, fromY);
					var textArea = new RectangleF(textLocation, textSize);
		...
		private void DrawFourthLayer(Graphics g, Rectangle r)
		{
			...
			int cx = mousecurrent.X - mousestartpoint.X;
			if (cx < 0) cx *= -1;
			int cy = mousecurrent.Y - mousestartpoint.Y;
			if (cy < 0) cy *= -1;
		...
		{
			double cdatox = P1X - P2X;
			if (cdatox < 0) cdatox *= -1;
			double cdatoy = P1Y - P2Y;
			if (cdatoy < 0) cdatoy *= -1;
