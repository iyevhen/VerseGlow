...
namespace VerseFlow.UI
{
	public class TextCursor
	{
		...
		private Int32 cHeight = 24;
		private Int32 cWidth = 24;
		...
		public String CursorText
		{
			...
			set
			{
				if (cursortext != value)
				{
					...
					var imgColor = new Bitmap(cWidth, cHeight, PixelFormat.Format32bppArgb);
					using (Graphics g = Graphics.FromImage(imgColor))
					{
						SizeF size = g.MeasureString(cursortext, font);
						cWidth = (Int32)size.Width;
						cHeight = (Int32)size.Height;
						g.Flush();
					}
		...
		{
			var imgColor = new Bitmap(cWidth, cHeight + 16, PixelFormat.Format32bppArgb);
			var rect = new RectangleF(0, 0, cWidth, cHeight);
			...
			var pgb = new PathGradientBrush(gp)
				{
					CenterPoint = new PointF(cWidth/2, cHeight/2),
					CenterColor = Color.Black,
					SurroundColors = new[] {xwhite}
				};
			...
			var cursorinfo = new IconInfo
				{
					...
					xHotspot = (cWidth/2),
					yHotspot = (cHeight/2),
					hbmMask = imgColor.GetHbitmap(),
					hbmColor = imgColor.GetHbitmap()
				};
			unsafe
			{
