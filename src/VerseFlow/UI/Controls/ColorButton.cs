...
namespace VerseFlow.UI.Controls
{
	public class ColorButton : System.Windows.Forms.Button
	{
			...
			using (Pen darkPen = new Pen(SystemColors.ControlDark))
			{
				...
				if (Enabled)
				{
					...
					using (var textFormat = new StringFormat())
					{
						...
						using (var textBrush = new SolidBrush(GraphicsTools.InvertColor(buttonColor)))
							e.Graphics.DrawString(hintText, Font, textBrush, rc, textFormat);
					}
				...
				using (var pen = new Pen(SystemColors.ControlLightLight))
					e.Graphics.DrawLine(pen, rc.Right + 5, rc.Top, rc.Right + 5, rc.Bottom);

				using (Pen textPen = new Pen(Enabled ? SystemColors.ControlText : SystemColors.GrayText))
				{
		...
		private class ColorPanel : System.Windows.Forms.Form
		{
			...
			private static readonly Color[] colorList = new Color[40]
        {
			...
			protected override void OnPaint(PaintEventArgs e)
			{
				...
				using (SolidBrush lightBrush = new SolidBrush(SystemColors.ControlLightLight))
				{
					bool selected = false;
					int x = 6, y = 5;
					...
					for (int i = 0; i < 40; i++)
					{
						...
						using (var brush = new SolidBrush(colorList[i]))
							e.Graphics.FillRectangle(brush, x, y, 11, 11);
			...
			protected override void OnClick(EventArgs e)
			{
				...
				Close();
				Dispose();
				colorButton.OnSelectedColorChanged(EventArgs.Empty);
			}
									   ...
									   int index, bool selected)
			{
				using (Pen darkPen = new Pen(SystemColors.ControlDark))
				using (Pen lightPen = new Pen(SystemColors.ControlLightLight))
				using (var lightBrush = new SolidBrush(SystemColors.ControlLightLight))
				{
					if (colorIndex == index)
					{
